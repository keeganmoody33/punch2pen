...
- **IPCClient** — JUCE Thread in the plugin managing the TCP connection to the engine. Completes Handshake/HandshakeResponse, drains the AudioRingBuffer, sends AudioChunk/TransportStop/Correction messages, and dispatches incoming TranscriptionResult events to listeners. On Mac it launches the packaged `punch2penEngine.app` via Launch Services (nested helper, then `/Applications/Punch2Pen`) so a Logic AU sandbox does not inherit onto the helper. It does not auto-launch leftover `~/punch2pen/bin/punch2penEngine` (`plugin/Source/IPCClient.h`).
...
- **Profile JSON** — `~/.punch2pen/profile_default.json` — JSON profile generated by ProfileManager to persist per-user corrections and vocabulary between engine runs.
